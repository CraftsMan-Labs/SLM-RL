...
def make_runner(tmp_path, monkeypatch, champ_primary, cand_primary, collapse=False,
    ...
    cfg = load_run_config(game="boxing", overrides={
    ...
    monkeypatch.setattr(
        "slm_rl.teachers.make_teacher",
        lambda game_cfg, seed=None, dqn_checkpoint=None: (
            RandomAgent(seed=seed), "teacher:boxing_puncher",
        ),
    )
    ...
    runner.game_cfg = runner.game_cfg.model_copy(update={"max_turns": 8})
...
def test_warm_start_teacher_generation(tmp_path, monkeypatch):
    ...
    assert manifest["rollout_model"] == "teacher:boxing_puncher"
    rollout_text = next(runner.paths.rollouts(1).glob("*.jsonl")).read_text()
    assert "teacher:boxing_puncher" in rollout_text
...
def test_no_eval_pruned_without_pruner(tmp_path, monkeypatch):
    # Keeper games have no menu pruner (make_pruner raises); default cfg
    # leaves teacher.pruner=False so eval_pruned stays absent.
    runner = make_runner(tmp_path, monkeypatch, champ_primary=0.10, cand_primary=0.40)
    runner.ensure_baseline()
    m = runner.run_generation(1)
    assert "eval_pruned" not in m
    assert runner.pruner is None
...
def test_rollout_batch_size_routes_through_batch_runner(tmp_path, monkeypatch):
    ...
    runner = make_runner(
        ...
        run_overrides={"backend": "transformers"},  # force batching-eligible
        # regardless of the machine running this test: auto-detect (plan
        # 024) can resolve to any-8gb/transformers on a host with no visible
        # CUDA (e.g. a container run without --gpus), which always forces
        # _batch_size()==1 and would make this batching test a false pass
    ...
    # "ACTION: 1" wins on TinyGame; episodes end without crashing mid-batch
    outcomes = {json.loads(line)["outcome"] for line in lines if json.loads(line)["outcome"]}
    assert outcomes <= {"win", "loss", "truncated"}
...
def test_config_dir_threads_to_game_config(tmp_path, monkeypatch):
    ...
    (cfg_dir / "games" / "boxing.yaml").write_text(
        "max_turns: 7\n", encoding="utf-8"
    )
    ...
    assert runner.game_cfg.max_turns == 7  # the override, not the repo's 2500

    repo_value = load_game_config("boxing").max_turns
