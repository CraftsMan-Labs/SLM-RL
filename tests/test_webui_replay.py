...
def test_replay_unavailable_without_env_id(tmp_path: Path, monkeypatch) -> None:
    from slm_rl.config.schema import GameConfig

    run_dir = tmp_path / "run1"
    rec = _base_record(game="text-only", episode_id="ep-x", parsed_action="A")
    path = run_dir / "generations" / "gen_000" / "rollouts" / "text-only.jsonl"
    ...
    with pytest.raises(ReplayUnavailable):
        list(replay_frames(run_dir, "ep-x"))


def test_replay_missing_episode_yields_nothing(tmp_path: Path) -> None:
    run_dir = tmp_path / "run1"
    _make_space_invaders_records(run_dir, episode_id="ep1")
    # stop already set => one catch-up pass, no live poll (would hang forever).
    stop = threading.Event()
    stop.set()
    assert list(replay_frames(run_dir, "does-not-exist", stop=stop)) == []
...
def test_frames_missing_extra_returns_501(tmp_path: Path, monkeypatch) -> None:
    ...
    def _boom(run_dir, episode_id, stop=None, *, generation=None, follow=True):
...
def test_frames_unavailable_game_returns_501(tmp_path: Path, monkeypatch) -> None:
    from slm_rl.config.schema import GameConfig
    ...
    try:
        conn = http.client.HTTPConnection("127.0.0.1", port, timeout=5)
        conn.request("GET", "/frames?episode=ep-x")
        resp = conn.getresponse()
        resp.read()
        assert resp.status == 501
    finally:
