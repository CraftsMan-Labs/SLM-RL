...
def test_rollout_batch_size_override_lands_in_train_config():
    cfg = load_run_config(game="boxing", overrides={"train": {"rollout_batch_size": 4}})
    assert cfg.train.rollout_batch_size == 4


def test_rollout_batch_size_omitted_keeps_default():
    cfg = load_run_config(game="boxing")
...
def test_selection_quantile_override_lands_in_train_config():
    cfg = load_run_config(game="boxing", overrides={"train": {"selection_quantile": 0.5}})
...
def test_grpo_budget_defaults_and_overrides():
    cfg = load_run_config()
    assert cfg.train.grpo_max_steps is None
    assert cfg.train.grpo_max_prompts == 512
    capped = load_run_config(
        overrides={"train": {"grpo_max_steps": 24, "grpo_max_prompts": 32, "group_size": 2}}
    )
    assert capped.train.grpo_max_steps == 24
    assert capped.train.grpo_max_prompts == 32
    assert capped.train.group_size == 2


def test_gate_stub_primary_defaults_to_minus_1_9():
    cfg = load_run_config()
    assert cfg.gate.stub_primary == -1.9
...
def test_game_config_missing_file_uses_defaults(tmp_path: Path):
    cfg = load_game_config("boxing", config_dir=tmp_path)
    assert cfg.name == "boxing"
...
def test_tier_table_parses_and_ends_with_universal_floor():
    ...
    assert floor.backend == "transformers"
    assert floor.train == "grpo"  # same stack as mac/CUDA tiers
