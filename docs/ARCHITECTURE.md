...
2. **Agents & Inference** (`slm_rl/agents/`, `slm_rl/inference/`) — an `Agent` turns observations into `ActionDecision`s via a pluggable `InferenceBackend` (transformers CUDA/MPS/CPU, optional MLX, vLLM). Hardware tier detection (`platform/hardware.py`) selects model + backend from `configs/hardware.yaml`.
...
- Default 8GB model: `LFM2.5-350M` (text) / `LFM2.5-VL-450M` (vision) via `transformers` (same GRPO stack as mac/CUDA; MLX remains an optional override).
...
## Generation theater (`slm-rl theater`)

`slm_rl/theater/exhibition.py` answers a question the on-disk layout above
otherwise can't: `run_suite` (the eval gate) never writes a rollout record,
so the base model (gen 0) has no replayable episodes anywhere, and there is
no direct "play base vs champion side by side" view. The exhibition runner
plays a small number of seeded episodes (`slm-rl theater --run-id <id>`,
10 by default) with the base model, releases that backend (8GB rule: one
model resident at a time), then plays the SAME seeds with the run's
champion adapter hot-swapped onto the same base weights. Seeds start at
`20_000` — disjoint from both the eval suite's seeds (`eval_seeds_start`,
10_000 by default) and the rollout formula (`cfg.seed + generation *
episodes_per_generation + i`), which stays far below 20_000 for every
shipped config.

**The zero-viewer-change trick.** Both sides are written via the same
`RolloutWriter` used everywhere else, under synthetic run dirs that mimic
the exact layout the viewer already globs:

```
<run_dir>/theater/base/generations/gen_000/rollouts/<game>.jsonl
<run_dir>/theater/champion/generations/gen_NNN/rollouts/<game>.jsonl
```

A `theater/<side>/` directory IS a valid viewer `run_dir` by construction,
so `slm_rl/webui/tailer.py` and `replay.py` need no changes at all to serve
it — `slm-rl watch --run <id>/theater/base` (or the playground's `/theater/
<name>/<side>/` mount) just works. Re-running an exhibition deletes and
recreates each side directory rather than appending (the same
partial-rollout lesson that shaped `RolloutWriter`'s callers elsewhere). A
run whose champion is still 0 (nothing ever promoted) gets a base-only
exhibition with a clear message instead of a synthetic, misleading
champion side.

**All-generations grid.** `webui/server.py::serve_events` grew an optional
`generation` filter (server-side, so unfiltered callers see byte-identical
behavior) exposed as `?gen=N` on both the standalone viewer and the
playground's `/watch/<name>/events`. The playground's `/gens/<name>/` page
renders one panel per generation present in the run (discovered by
globbing `generations/gen_*`), each an iframe of the ordinary viewer page
pointed at `?gen=N` — no new viewer routes, just query-string forwarding
that `webui/page.py`'s JS already does from the page's own URL to the
`events`/`frames` calls it makes. Atari frame replay stays capped at 4
concurrent streams server-wide (unchanged); grid panels beyond that show
the text stream only.
...
**Signup + publish (plan 021).** `slm_rl/playground/profile.py` stores one
local, offline-writable profile at `<home>/playground/profile.json`
(`{name, hf_username, hf_token, created_at}`, file mode `0600` set at
creation via `os.open`, never chmod-after). `GET /api/profile` 404s until
signup, then returns a MASKED view (`token_masked` is the last 4 chars
only; the full token never leaves this module except to a caller about to
pass it straight to `huggingface_hub` as `token=`). `POST /api/profile`
saves (name required, token optional — "skip for now" is a name-only save,
never a lockout: rollouts/evolve/theater don't need HF). `hf_username` is
resolved lazily (`HfApi(token=...).whoami()`) at first PUBLISH, not at
signup, so signup stays fully offline.

`POST /api/experiments/<name>/publish` (`slm_rl/datagen/hf_publish.py`) has
two independent, separately-reported sides: the dataset side reuses
`hf_push.push_generation` per generation dir, repo
`{username}/slm-rl-<experiment>-data`; the model side resolves the run's
champion generation the same way `slm_rl/theater/exhibition.py` does
(`ModelRegistry(...).champion` + `RunPaths(...).adapter(gen)`), uploads
`adapter/` plus a generated model card (game, base model from
`MANIFEST.json`, champion generation, gate metrics from `metrics.json`) to
repo `{username}/slm-rl-<experiment>`. Champion 0 (no promotion yet)
publishes datasets only, with a message explaining why — no adapter
directory to upload. Every `huggingface_hub` call takes `token=` explicitly
(never ambient auth) since this publishes to the ATTENDEE's own account.
Publish runs inline in the request thread (I/O-bound uploads, not a
subprocess), guarded by a per-experiment-name lock so a double click can't
launch two uploads of the same experiment at once.
