...
CHAPTERS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "title": "The games",
        "heading": "## 1. The games",
        "goal": "See Atari as text: RAM in, a numbered action menu out. Play the title yourself before any model does.",
    ...
    },
    {
        ...
        "goal": "Walk the DQN loop, watch Mario train live, evaluate a trained policy, then train the mute Atari teacher. Recorded clips stay the fallback.",
        "slides": "section-dqn → what-is-dqn → dqn-q-values → dqn-bellman → dqn-replay → dqn-target → dqn-epsilon → dqn-curve → dqn-live-train → dqn-live-eval → dqn-bridge → teacher-dataset",
        "slide_ids": (
            "section-dqn",
            "what-is-dqn",
            "dqn-q-values",
    ...
    },
    {
        ...
        "title": "Packs",
        "heading": "## 6. Packs",
        "goal": "Inspect a raw teacher trace, convert it to an SFT pair, then bake a shareable pack.",
        "slides": "section-packs → synthetic-homework → trace-to-pair → dataset-filters → why-warmstart",
        "slide_ids": (
            "section-packs",
            "synthetic-homework",
            "trace-to-pair",
