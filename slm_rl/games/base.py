...
class Game(ABC):
    ...
    def export_exempt(cls) -> bool:
        """True if this game's rollout records must never be fed to
        `datagen/sft_export.py` (CODING_GUIDELINE invariant: vision records
        must not poison text SFT, plan 011). Default False -- current
        behavior unchanged for every existing game. A classmethod (not an
        instance check) so the export path can decide per-`game` name
        without constructing a `Game` instance from a bare dataset row."""
        return False
