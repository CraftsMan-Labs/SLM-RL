...
from slm_rl.games.reward_hook import RewardHook
...
class GymnasiumGameAdapter(Game):
    ...
    def __init__(
        ...
        self._reward_hook = RewardHook(extra.get("reward_hook"))
    ...
    def reset(self, seed: int | None = None) -> Observation:
        ...
        if self.noop_start_max > 0 and seed is not None:
            ...
            for ram, _reward, terminated, truncated, info in self._iter_noop_start(
                env, seed, self._action_ids
            ):
    ...
    def _iter_noop_start(self, env, seed: int, action_ids: list[str]):
        """Yield env.step results for the seeded no-op start prefix."""
        rng = random.Random(seed * 10_007 + 11)
        k = rng.randint(0, self.noop_start_max)
        # ponytail: assumes action 0 is a no-op-like action when the env
        # doesn't expose "NOOP" explicitly.
        noop_id = action_ids.index("NOOP") if "NOOP" in action_ids else 0
        for _ in range(k):
            result = env.step(noop_id)
            yield result
