...
def group_episodes(dataset_path: Path) -> dict[str, list[dict]]:
    """Group records by episode_id, each episode sorted by step_idx."""
...
def select_episodes(dataset_path: Path, cfg: TrainConfig) -> list[list[dict]]:
    ...
    episodes = group_episodes(dataset_path)
...
def filter_rollouts_top_quantile(
    dataset_path: Path,
    out_path: Path | None = None,
    *,
    selection_quantile: float = 0.25,
    exclude_monitor_flagged: bool = True,
    max_duplicate_action_sequences: int = 3,
    win_turn_cap: int = 0,
) -> dict[str, Any]:
    """Rewrite a rollouts jsonl keeping only `select_episodes` survivors.

    Used at bake time so packs ship the top-quantile demos (by return / wins)
    instead of every teacher episode. `selection_quantile >= 1.0` keeps all
    episodes (no rewrite when `out_path` is None / same as `dataset_path`).
    """
    src = Path(dataset_path)
    dest = Path(out_path) if out_path is not None else src
    n_raw = len(group_episodes(src))
    if n_raw == 0:
        return {
            "n_raw": 0,
            "n_kept": 0,
    ...
    if selection_quantile >= 1.0:
        if dest.resolve() != src.resolve():
            dest.parent.mkdir(parents=True, exist_ok=True)
            dest.write_bytes(src.read_bytes())
        n_steps = sum(1 for _ in _iter_records(src))
        return {
            "n_raw": n_raw,
            "n_kept": n_raw,
            "n_steps": n_steps,
    ...
    cfg = TrainConfig(
        selection_quantile=selection_quantile,
        exclude_monitor_flagged=exclude_monitor_flagged,
        max_duplicate_action_sequences=max_duplicate_action_sequences,
        win_turn_cap=win_turn_cap,
    )
    selected = select_episodes(src, cfg)
    dest.parent.mkdir(parents=True, exist_ok=True)
...
def _is_export_exempt(game_name: str) -> bool:
    """Plan 011 hard rule 3 (vision records must not poison text SFT):
    `Game.export_exempt()` defaults False (games/base.py) so this is a
    no-op for every text-native game; a vision (or otherwise non-text)
    game overrides it True. A game name the registry can't resolve
    (extra not installed, typo) is treated as NOT exempt -- current
    export behavior, not a new failure mode -- since a truly-exempt game
    would only ever appear in a dataset if it was actually played, which
    requires its extra to be installed."""
    from slm_rl.games.registry import get_game

    try:
        return get_game(game_name).export_exempt()
    except KeyError:
        return False
...
def export_sft_dataset(dataset_path: Path, out_path: Path, cfg: TrainConfig) -> int:
    ...
    Episodes from an export-exempt game (plan 011: vision rollouts, whose
    `prompt_messages` carry an image placeholder rather than trainable
    text) are skipped entirely, with a loud count -- never silently
    ingested (CODING_GUIDELINE Sec 4.5: log what a cap/filter drops)."""
    ...
    with open(out_path, "w", encoding="utf-8") as fh:
        for steps in select_episodes(dataset_path, cfg):
            # .get(...): readers tolerate missing keys (CODING_GUIDELINE Sec
            # 4.3) -- a record without "game" (old data, or a hand-built
            # test fixture) is simply never exempt, current behavior.
            if _is_export_exempt(steps[0].get("game", "")):
                skipped_exempt += 1
                continue
            ...
            for rec in steps:
                ...
                if not prompt:
                    continue
                # ponytail: heuristic teacher completions carry a verbalized
                # rationale (plan 002) and are trusted verbatim. DQN teachers
                # stamp "Q-values rank X highest (...)" — those numbers are
                # not reproducible by the SLM at inference, so the student
                # learns to hallucinate a sticky ranking and mode-collapse
                # (always UP / always RIGHT). Strip to ACTION-only for those.
                # Model-generated completions can contain retry junk, so
                # those are still rebuilt from the canonical parsed_action.
                is_teacher = str(rec.get("model_id", "")).startswith("teacher:")
                if is_teacher:
                    raw = rec["completion"]
                    text = raw if isinstance(raw, str) else str(raw)
                    if "Q-values rank" in text:
                        content = f"ACTION: {rec['parsed_action']}"
                    else:
                        content = text
                else:
                    content = f"ACTION: {rec['parsed_action']}"
    ...
    if skipped_exempt:
        ...
        warnings.warn(
            f"export_sft_dataset: skipped {skipped_exempt} export-exempt "
            f"episode(s) (e.g. vision records) -- never converted to "
            "trainable text",
            stacklevel=2,
        )
    return pairs
